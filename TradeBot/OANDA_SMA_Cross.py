# -*- coding: utf-8 -*-
"""
Created on Thu Jul 16 21:49:49 2020
...
#expect about 5 seconds of latency
my_token = ""
...
def candles(instrument):
    params = {"count": 400,"granularity": "M5"} #granularity can be in seconds S5 - S30, minutes M1 - M30, hours H1 - H12, days D, weeks W or months M
...
def trade_signal(df,curr):
    ...
    if dnward_sma_dir[curr] == True and min(df['K'][-1],df['D'][-1]) > 75 and max(df['K'][-2],df['D'][-2]) < 75:
        signal = "Sell"
    #these lines help visualize the crossover    
    # plt.subplot(211)
...
###########################Execution########################################## 
starttime=time.time()
timeout = time.time() + 60*11*1  # 60 seconds times 60 meaning the script will run for 1 hr
while time.time() <= timeout:
    try:
        print("passthrough at ",time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
        main()
        #make sure that this sleep time coincides with the historical data time interval
